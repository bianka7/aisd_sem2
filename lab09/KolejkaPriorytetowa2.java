...
public class KolejkaPriorytetowa2 {
    ...
    public KolejkaPriorytetowa2(int capacity) {
    ...
    private void swim(int k) {
        while (k > 1 && (rq[k / 2].compareTo(rq[k]) > 0)) {
            swap(k, k / 2);
            k = k / 2;
        }
    }
    ...
    public void heapSort() {
        for (int i = n / 2; i >= 1; i--) {
            sink(i); // budowanie kopca
        }
        int temp = n;
        while (n > 1) { // sortowanie
            swap(1, n--);
            sink(1);
        }
        n = temp; //do przechowania faktycznego rozmiaru kopca, do wyswietlania
    }
    public void print() {
        for (int i = 1; i <= n; i++) {
            if (rq[i] != null) {
    ...
    public static void main(String[] args) {
        KolejkaPriorytetowa2 kolejka = new KolejkaPriorytetowa2(8);
